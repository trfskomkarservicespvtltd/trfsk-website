...
export default async function AdminPage() {
  ...
  return <div className="space-y-10"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-amber-300">Operations</p><h1 className="mt-2 text-4xl font-semibold text-white">Partner control room</h1><p className="mt-2 text-slate-400">Partner accounts are created automatically. Manage auditable financial activity and monthly returns here.</p></div><div className="grid gap-4 md:grid-cols-2"><Metric label="Partner accounts" value={String(accountOptions.length)} /><Metric label="Ledger entries" value={String(entries ?? 0)} /></div><AdminForms accounts={accountOptions} /></div>;
}
