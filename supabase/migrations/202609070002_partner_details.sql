...
drop policy if exists "Partners manage own details" on public.partner_details;
...
drop policy if exists "Partners manage own documents" on storage.objects;
create policy "Partners manage own documents" on storage.objects
  for all
  using (
